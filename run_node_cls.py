...
if __name__ == '__main__':
    ...
    trainer = NodeClsTrainer(data, model, params, niter=1, verbose=args.verbose)
    trainer.run()
    ...
    def add_noisy_features(data, noise_level):
        noisy_data = data.clone().to(device)
        a, b = data.features.shape
        # num_to_drop = int(b * noise_level)
        # noisy_indices = torch.randperm(b)[:num_to_drop]
        noisy_data.features += (feature_mean[dataset_str] + 2 * torch.randn(a, b) * np.sqrt(
            feature_std[dataset_str])) * noise_level
        # print("orig data", data.features.detach().numpy()[:10])
        # print("clone noisy data", noisy_data.features.detach().numpy()[:10])
        return noisy_data
    ...
    def compute_margin(log_probs):
        ...
        exp.sort(dim=1, descending=True)
        return exp[:, 0] - exp[:, 1]
    ...
    def plt_res(name):
        ...
        plt.plot(
            [sigma for sigma in noise_levels],
    ...
    data = list(zip(noise_levels, [res['PU'] for res in results], [res['AU'] for res in results],
